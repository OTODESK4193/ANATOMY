...
with open('PluginProcessor.cpp', 'r', encoding='utf-8') as f:
    text = f.read()

block1 = '''    juce::AudioBuffer<float> localTrans, localTonal, localLayer;
    double sr = 44100.0;
...
text = re.sub(r'    juce::AudioBuffer<float> localTrans.*?    for \(int ch = 0; ch < 2; \+\+ch\).*?    }', block1, text, flags=re.DOTALL)

block2 = '''    applyPitch(workTonal, tonalPitch);
...
text = re.sub(r'    applyPitch\(workTonal, tonalPitch\);.*?    outTonalRendered\.clear\(\);', block2, text, flags=re.DOTALL)

block3 = '''    processor.applyEffectsOffline(workTrans, TargetRoute::Transient, sr);
...
text = re.sub(r'    processor\.applyEffectsOffline\(workTrans.*?componentRatios = std::move\(ratios\);\n    }', block3, text, flags=re.DOTALL)

with open('PluginProcessor.cpp', 'w', encoding='utf-8') as f:
    f.write(text)
