...
public:
    TonalLaneView(AnatomyAudioProcessor& p) : processor(p)
    {
        ...
        styleBtn(prevBtn,   AnatomyColors::accentTonal);
        styleBtn(nextBtn,   AnatomyColors::accentTonal);
        styleBtn(resetBtn,  AnatomyColors::textDim);

        auto sName = processor.getCustomSampleName(2);
        ...
        resetBtn.onClick = [this] {
            ...
            processor.setCustomSampleName(2, {});
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        g.drawText("TONAL", 12, 6, getWidth() - 320, 20, juce::Justification::centredLeft);
...
private:
    ...
    {
        juce::File startDir;
        auto lastFile = processor.getLastLoadedFile(2);
    ...
    void loadFile(const juce::File& file)
    {
        ...
        if (reader != nullptr)
        {
            ...
            processor.setLastLoadedFile(2, file);
            auto name = file.getFileNameWithoutExtension().substring(0, 8);
            processor.setCustomSampleName(2, name);
    ...
    {
        auto file = processor.getNeighborAudioFile(2, isNext);
        if (file.existsAsFile())
            loadFile(file);
    }
